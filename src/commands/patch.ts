...
export const patch: Command = {
  ...
  async exec(ctx) {
    ...
    try {
      ...
      if (inputFile) {
        const patchPath = ctx.fs.resolvePath(inputFile, ctx.cwd);
        patchContent = await readFileText(ctx.fs, patchPath);
      } else if (positional.length > 0) {
        const patchPath = ctx.fs.resolvePath(positional[0], ctx.cwd);
        patchContent = await readFileText(ctx.fs, patchPath);
      } else {
        patchContent = ctx.stdin;
      }
      ...
      for (const patch of patches) {
        ...
        if (!dryRun) {
          ...
          try {
            const filePath = ctx.fs.resolvePath(targetFile, ctx.cwd);
            originalContent = await readFileText(ctx.fs, filePath);
          } catch {
          ...
          // Write result
          if (outputFile) {
            const outPath = ctx.fs.resolvePath(outputFile, ctx.cwd);
            await ctx.fs.writeFile(outPath, patched);
          } else {
            const filePath = ctx.fs.resolvePath(targetFile, ctx.cwd);
            await ctx.fs.writeFile(filePath, patched);
          }
        }
      }
      ...
      ctx.stderr += `patch: ${e instanceof Error ? e.message : e}\n`;
      return 1;
    }
  },
};
