...
`;
    ...
    const stat = await ctx.fs.stat(projectDir);
    ...
    router.post('/api/fs/read', async (req) => {
      ...
      try {
        ...
        return { status: 200, body: JSON.stringify({ content }), headers: { 'Content-Type': 'application/json' } };
      } catch (e: any) {
        return { status: 404, body: JSON.stringify({ error: e.message }), headers: { 'Content-Type': 'application/json' } };
      }
    });
    ...
    router.post('/api/fs/readdir', async (req) => {
      ...
      try {
        ...
        return { status: 200, body: JSON.stringify({ entries }), headers: { 'Content-Type': 'application/json' } };
      } catch (e: any) {
        return { status: 500, body: JSON.stringify({ error: e.message }), headers: { 'Content-Type': 'application/json' } };
      }
    });
    ...
    router.post('/api/fs/stat', async (req) => {
      ...
      try {
        ...
        if (!s) return { status: 404, body: JSON.stringify({ error: 'Not found' }), headers: { 'Content-Type': 'application/json' } };
        return { status: 200, body: JSON.stringify({ type: s.type, size: s.size, mtime: s.mtime }), headers: { 'Content-Type': 'application/json' } };
      } catch (e: any) {
        return { status: 500, body: JSON.stringify({ error: e.message }), headers: { 'Content-Type': 'application/json' } };
      }
    });
        ...
        if (recursive) {
          ...
          if (s?.type === 'dir') {
            ...
            for (const name of entries) {
              ...
              if (cs?.type === 'dir') {
                // Recurse via shell for simplicity
    ...
    router.post('/api/fs/glob', async (req) => {
      ...
      try {
        ...
        return { status: 200, body: JSON.stringify({ files }), headers: { 'Content-Type': 'application/json' } };
      } catch (e: any) {
        return { status: 500, body: JSON.stringify({ error: e.message }), headers: { 'Content-Type': 'application/json' } };
      }
    });
    ...
    router.post('/api/shell/exec', async (req) => {
      ...
      try {
        ...
        return {
          status: 500,
          body: JSON.stringify({ stdout, stderr: stderr + e.message, exitCode: 1 }),
          headers: { 'Content-Type': 'application/json' },
        };
      }
    });
    ...
    // ── Git routes ──
    router.post('/api/git/status', async (req) => {
      let stdout = '', stderr = '';
      const exitCode = await shell.execute(
        `cd "${projectDir}" && git status --porcelain`,
        (d: string) => { stdout += d; }, (d: string) => { stderr += d; },
      );
      return { status: 200, body: JSON.stringify({ stdout, stderr, exitCode }), headers: { 'Content-Type': 'application/json' } };
    });
    ...
    router.post('/api/git/push', async (req) => {
      let stdout = '', stderr = '';
      const exitCode = await shell.execute(
    ...
    router.post('/api/git/pull', async (req) => {
      let stdout = '', stderr = '';
      const exitCode = await shell.execute(
    ...
    router.post('/api/project/scaffold', async (req) => {
      ...
      if (!tmpl) {
        return { status: 400, body: JSON.stringify({ error: 'Unknown template: ' + template }), headers: { 'Content-Type': 'application/json' } };
      }
      ...
      try {
        ...
        return { status: 200, body: JSON.stringify({ ok: true, dir: targetDir }), headers: { 'Content-Type': 'application/json' } };
      } catch (e: any) {
        return { status: 500, body: JSON.stringify({ error: e.message }), headers: { 'Content-Type': 'application/json' } };
      }
    });
